...
async def book_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Переключает бота в режим поиска по книгам для следующего запроса (однократно).
    """
    user = update.effective_user
    user_id = user.id if user else 0

    try:
        logger.info("Пользователь %s вызвал команду /book", user_id)
        await set_typing_action(update, context)

        set_user_ephemeral_mode(user_id, "book")

        text = (
            "Следующий поиск будет <b>только по книгам</b> <i>(однократно)</i>."
        )
        ...
        logger.exception("Ошибка в команде /book для пользователя %s: %s", user_id, e)
