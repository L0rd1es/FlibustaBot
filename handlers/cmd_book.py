...
async def book_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Переключает бота в режим поиска по книгам для следующего запроса.
    ...
    try:
        logger.info(f"Пользователь {user_id} вызвал команду /book")
        ...
        logger.error(f"Ошибка в команде /book для пользователя {user_id}: {e}")
        if update.message:
